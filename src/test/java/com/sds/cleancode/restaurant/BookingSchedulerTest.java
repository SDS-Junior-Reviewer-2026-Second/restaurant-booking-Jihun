...
public class BookingSchedulerTest {
    ...
    private Customer FAKE_CUSTOMER = new Customer("Fake name","010-1234-1111");
    private Customer FAKE_CUSTOMER_WITH_MAIL = new Customer("Fake name","010-1234-1111", "abcd1234@naver.com");
    ...
    @BeforeEach
    void setUp() {
        bookingScheduler = new TestableBookingScheduler(CAPACITY_PER_HOUR, MONDAY_DATE);
    ...
    public void 예약은_정시에만_가능하다_정시가_아닌경우_예약불가() {
        Schedule schedule = new Schedule(NOT_ON_THE_HOUR, UNDER_CAPACITY, FAKE_CUSTOMER);
    ...
    public void 예약은_정시에만_가능하다_정시인_경우_예약가능() {
        Schedule schedule = new Schedule(ON_THE_HOUR, UNDER_CAPACITY, FAKE_CUSTOMER);
    ...
    public void 시간대별_인원제한이_있다_같은_시간대에_Capacity_초과할_경우_예외발생() {
        Schedule schedule = new Schedule(ON_THE_HOUR, CAPACITY_PER_HOUR, FAKE_CUSTOMER);
        Schedule schedule2 = new Schedule(ON_THE_HOUR, 1, FAKE_CUSTOMER);
        assertThrows(RuntimeException.class, ()-> {
            bookingScheduler.addSchedule(schedule);
    ...
    public void 시간대별_인원제한이_있다_같은_시간대가_다르면_Capacity_차있어도_스케쥴_추가_성공() {
        Schedule schedule = new Schedule(ON_THE_HOUR, CAPACITY_PER_HOUR, FAKE_CUSTOMER);
        Schedule schedule2 = new Schedule(ON_THE_HOUR.plusHours(1), CAPACITY_PER_HOUR , FAKE_CUSTOMER);
    ...
    public void 이메일이_없는_경우에는_이메일_미발송() {
        ...
        assertEquals(0, testableMailSender.getSendMethodCount());
    }

    @Test
    public void 이메일이_있는_경우에는_이메일_발송() {
        Schedule schedule = new Schedule(ON_THE_HOUR, CAPACITY_PER_HOUR, FAKE_CUSTOMER_WITH_MAIL);
        bookingScheduler.addSchedule(schedule);

        assertEquals(1, testableMailSender.getSendMethodCount());
    }

    @Test
    public void 현재날짜가_일요일인_경우_예약불가_예외처리() {

        bookingScheduler = new TestableBookingScheduler(CAPACITY_PER_HOUR, SUNDAY_DATE);

        RuntimeException exception = assertThrows(
                RuntimeException.class,
                () -> bookingScheduler.addSchedule(new Schedule(ON_THE_HOUR, UNDER_CAPACITY, FAKE_CUSTOMER))
        );
